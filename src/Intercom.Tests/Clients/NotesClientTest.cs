...
namespace Intercom.Test
{
    ...
    public class NotesClientTest : TestBase
    {
        ...
        {
            Assert.Throws<ArgumentException>(() => notesClient.Create(new Note() { user = new User() { email = "email@example.com" } }));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => notesClient.Create(new User() { email = "email@example.com" }, String.Empty));
        }
